...
namespace Avvera
{
    internal class Avvera
    {
        ...
        static void Main(string[] args)
        {
            ...
            XFont bold = new XFont("Tahoma", 14, XFontStyleEx.Bold);
            XFont mini_bold = new XFont("Tahoma", 10, XFontStyleEx.Bold);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    string denominazione = riga_letta[3].ToUpper();
                    ...
                    foreach (char c in denominazione)
                    {
                        switch ((int)c)
                        {
                            case int x when x == 193 |
                                            x == 192:
                                denominazione = denominazione.Replace(Encoding.Default.GetString(new byte[] { (byte)x }), "A'");
                                break;

                            case int x when x == 200 |
                                            x == 201:
                                denominazione = denominazione.Replace(Encoding.Default.GetString(new byte[] { (byte)x }), "E'");
                                break;

                            case int x when x == 204 |
                                            x == 205:
                                denominazione = denominazione.Replace(Encoding.Default.GetString(new byte[] { (byte)x }), "I'");
                                break;

                            case int x when x == 210 |
                                            x == 211:
                                denominazione = denominazione.Replace(Encoding.Default.GetString(new byte[] { (byte)x }), "O'");
                                break;

                            case int x when x == 217 |
                                            x == 218:
                                denominazione = denominazione.Replace(Encoding.Default.GetString(new byte[] { (byte)x }), "U'");
                                break;
                        }
                    }

                    string codice_fiscale = riga_letta[6];
                    ...
                    if (riga_letta[7] != "")
                    {
                        data_nascita = DateTime.Parse(riga_letta[7]).ToString("dd/MM/yyyy");
                    }
                    string denom_soc = riga_letta[8].ToUpper();
                    if (nome == "")
                    {
                        nome = denom_soc;
                    }
                    string ric_documenti = riga_letta[15];
                ...
                ivass:
                    ...
                    //LogFile pdf_ivass = new LogFile(ut.CurDir() + "\\PDF\\" + denominazione + "_" + codice_fiscale + "_RUI.txt");
                    //pdf_ivass.Write("+ " + denominazione);

                    Dictionary<string, string> dettaglio_ivass = new Dictionary<string, string>();
                    if (codice_fiscale.Length > 11)
                    {
                        dettaglio_ivass = Ivass.ricerca("", DateTime.Parse(data_nascita).ToString("dd/MM/yyyy"), nome, cognome);
                    }
                    else
                    {
                ...
                ocf:
                    ...
                    //var pdf_ocf = new LogFile(ut.CurDir() + "\\PDF\\" + denominazione + "_" + codice_fiscale + "_OCF.txt");

                    pdf.scrivi(denominazione, XBrushes.Black, true, bold);
                    //pdf_ocf.Write("+ " + denominazione);

                    //List<string> dettaglio_ocf = new List<string>();
                    ...
                    if (dettaglio_ocf.Count > 0)
                    {
                        ...
                        //pdf_ocf.Write("+ STORICO DEGLI STATI");
                ...
                prossimo:
                    excel.WriteExcel(riga_out, riga, 23);
